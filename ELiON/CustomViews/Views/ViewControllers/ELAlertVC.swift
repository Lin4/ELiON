...
class ELAlertVC: UIViewController {
    
    let containerView = ELAllertContainerView()
    let titleLabel = ELTitleLabel(textAlignment: .center, fontSize: 20)
    let messageLabel = ELBodyLabel(textAlignment: .center)
    let alertButton = ELButton(color: .systemCyan, title: "Ok", systemImageName: "checkmark.circle")
